...
"""
			...
			# Convert to SQLAlchemy Column syntax in `field_name = Column` format
			if 'string' in field_value:
				size = field_value.split('(')[1][:-1] if '(' in field_value else None
				if size:
					columns.append(f"{field_name} = Column(String({size}))")
				else:
					columns.append(f"{field_name} = Column(String)")
			elif 'integer' in field_value:
				columns.append(f"{field_name} = Column(Integer)")
	...
	with open(f"src/cargo/{name}Model.py", "w") as f:
		...
		f.write("""from libmercury.db import Column, Integer, String, Base\n""")
		if len(methods) > 0:
