...
def generate_hmac_key(name, path):
    ...
    print(f"HMAC secret key saved to {os.path.join(path, f'{name}Hmac_secret.key')}")
...
def generate_rsa_keys(name, path):
    ...
    print(f"RSA private key saved to {os.path.join(path, f'{name}Public_key.pem')}")
    print(f"RSA public key saved to {os.path.join(path, f'{name}Private_key.pem')}")
